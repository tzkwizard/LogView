(function () {
    ...
    angular.module('app')
        .controller(controllerId, function($timeout, $cookieStore, $rootScope, $scope, $location, common, bsDialog, client, datasearch, dataconfig) {
        ...
        vm.drawMap = drawMap;
          ...
          ['Beijing','China',23,22]
                ...
                vm.fieldstree = vm.fieldsName;
                 ...
                 data.addRow(["Elasticsearch", null, 0, 0]);
                 angular.forEach(vm.indicesName, function (n) {
           
                     data.addRow([n, "Elasticsearch", 0, 0]);                   
                     
                     angular.forEach(vm.fieldstree, function (m) {
                        // var x = Math.random() * 100 - 50;
                         data.addRow([m + "--" + n, n, 0,0]);
                         ...
                         client.search({
                             ...
                             type: "logs",
                             body: ejs.Request()
                                 .aggregation(ejs.TermsAggregation("agg").field(m))

                         }).then(function (resp) {
                             vm.total = resp.hits.total;
                             vm.nn = resp.aggregations.agg.buckets;
                             $timeout(treecount, 10);
                         }, function (err) {
                             log(err.message);
                         });

                         vm.treecount = treecount;
                        
                         function treecount(){
                         angular.forEach(vm.nn, function (cc) {
                             var x = Math.random() * 100 - 50;
                             data.addRow([cc.key.toString()+"--------"+m+n, m, cc.doc_count, x]);
                         });
                         }
                ...
                function showStaticTooltip(row, size, value) {
                    return '<div style="background:#fd9; padding:10px; border-style:solid">' +
                           '<a> Tag:' + data.getValue(row, 0)+'<hr>' + '</b> Size:' + size + '</b> Value:' + value + '</a>.</div>';
                }
